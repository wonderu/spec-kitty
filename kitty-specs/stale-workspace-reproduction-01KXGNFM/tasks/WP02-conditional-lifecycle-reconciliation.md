...
# Work Package Prompt: WP02 – Conditional Lifecycle Reconciliation
...
Consume WP01's current-base disposition and change the lane lifecycle seam only
when WP01 proves a lifecycle/reconciliation row RED. For an activated RED, make
...
or test edits, record the no-op evidence in the WP handoff, and send WP02 directly
...
real CLI witness and `disposition-matrix.md`; WP02 does not reinterpret, broaden,
or replace that evidence. It consumes only rows whose reached owner is the lane
...
and workspace match the returned contract and that WP01 is approved or done.
...
modules, workflow call sites, Mission metadata, or unrelated tests. WP03 owns any
review-call-site wiring needed after this seam has an evidence-backed contract.

### Mandatory decision gate

Before selecting a code path, extract the following from WP01's disposition for
...
| A lifecycle-owned row is RED/continue | Activate T007–T010 only for the proven state and behavior. |
...
### Subtask T006: Consume WP01 disposition and select no-op or remediation
...
1. Confirm WP01 is approved or done using the canonical task status surface.
   Do not rely only on the presence of a file or a verbal summary.
2. Read WP01's completed disposition matrix from the Mission planning surface.
...
6. On `NO-OP/GREEN`, compare both owned files before and after to prove they are
   ...
   skip all production-edit steps in T007–T009.
...
- Read only: WP01 disposition and Mission contract artifacts.
- On the no-op path: no files modified.
- On the activated path: subsequent subtasks may modify only the two owned files.

**Validation**:

- The handoff names the exact WP01 evidence and one unambiguous decision.
...
### Subtask T007: Add canonical readiness classification only for a proven RED
...
### Subtask T008: Thread one resolved identity through lifecycle operations
...
5. If call-site wiring is required outside this WP's owned files, stop at the
   narrow lifecycle contract and give WP03 an exact signature/behavior handoff.
...
### Subtask T009: Make refusal structured, actionable, and platform-neutral
...
7. Verify JSON serialization yields a single coherent payload when consumed by
   the owning command. Any command-envelope wiring belongs to WP03 unless the
...
### Subtask T010: Complete focused RED→GREEN and quality verification
...
- [ ] WP01 is approved/done and its lifecycle-owned disposition rows were consumed.
...
- [ ] Only owned files changed, and WP03 receives any required call-site handoff.
...
- **Unsafe recovery**: creating a directory or branch can conceal absent authority.
  ...
  worktree lifecycle.
- **Scope collision with WP03**: review call-site changes are not owned here.
...
6. The diff is confined to the two owned files; workflow wiring is deferred to
   WP03 and adjacent issues remain unclaimed.
