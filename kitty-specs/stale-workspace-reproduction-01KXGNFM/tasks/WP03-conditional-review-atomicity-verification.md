...
# WP03 — Conditional Review Atomicity and Verification
...
7. The completed WP01 and WP02 review evidence.
...
stale-workspace witness and the row-by-row `stop`/`continue` disposition. WP02 owns
conditional lifecycle reconciliation. WP03 consumes both outputs and changes
...
- **Dependencies**: WP01 and WP02 must both be `approved` or `done` before WP03 is
  claimed. Do not bypass dependency gating.
- **Start command**:

  ```bash
  spec-kitty agent action implement WP03 --agent codex --mission stale-workspace-reproduction-01KXGNFM
  ```
...
- Completed WP03 changes merge back to `fix/stale-workspace-reproduction`. The
...
### Subtask T011: Consume the disposition and preserve no-op GREEN arms

**Purpose**: Convert WP01 and WP02 evidence into an explicit authorization gate so
WP03 never performs speculative production work.
...
2. Read WP02's approved result and determine whether lifecycle reconciliation was:
   - activated and implemented for a proven RED;
   - characterized as already correct and left unchanged; or
   - blocked, in which case WP03 must not begin production edits.
3. Build a local implementation checklist mapping each review row to exactly one
   ...
   - `RED/continue — ordering`: activate T012 through T014;
   - `RED/continue — placement`: activate T015 after recording adjacency;
...
6. If all review rows are GREEN/stop, make no production changes. Run T016's
   aggregate verification, record the no-op disposition in the handoff, and finish.

**Files**:

- Read-only input: `disposition-matrix.md` and WP01/WP02 evidence.
...
- The handoff states whether T012–T015 were activated, skipped, or stopped.

### Subtask T012: Establish non-mutating readiness before claim
...
### Subtask T013: Track invocation-owned recovery and lock resources
...
5. For a recoverable husk, allow materialization only after WP02/canonical
...
### Subtask T014: Compensate invocation-owned resources on later failure
...
3. Add one structured cleanup path that runs for every post-readiness exception or
   refusal and receives the invocation ownership record from T013.
...
### Subtask T015: Handle a proven placement RED without absorbing issue #2160
...
2. Before production editing, tell the orchestrator to record a #2160-adjacent
   residual in `issue-matrix.md` and the DRAFT PR. WP03 does not own those planning
   surfaces and must not edit them directly.
...
- `issue-matrix.md` and PR text are orchestrator-owned follow-through, not WP03
  file edits.
...
### Subtask T016: Run aggregate GREEN gates and preserve the WP01 witness
...
1. Re-run WP01's acceptance witness unchanged. Compare its file content or commit
   identity with the approved WP01 version before running it; WP03 does not own and
...
6. Run focused regression suites named by WP01/WP02 for healthy workspace and lane
   lifecycle behavior, including WP02's integration module when WP02 activated.
...
8. Run the architectural checks relevant to shared-package boundaries, placement,
   ...
   Add any non-vacuous placement/authority guard identified by WP01 or WP02.
...
- [ ] WP01 and WP02 are approved/done and their dispositions were consumed before
      any WP03 production edit.
...
- [ ] Changes remain on the resolved WP03 lane and target
...
- **Speculative repair**: implementing an imagined review defect would violate
  FR-003. Mitigation: T011 makes the WP01 matrix an explicit authorization gate.
...
10. Does the branch/workspace evidence show the resolved WP03 lane targets
